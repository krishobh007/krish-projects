class ApiKeysController < ApplicationController
  ...
  # POST /api_keys.json
  def create
    # if not Time.now > Time.parse(params[:expiry_date])
    # raise "Expiry Date should be less than current date"
    # end
    @api_key = ApiKey.new(params[:api_key])
    @api_key.key = Digest::MD5.hexdigest("wizards#{Time.now}:killed:#{params[:email]}:my_family:#{rand(15_562)}")

    respond_to do |format|
      if @api_key.save
        ApiKeyMailer.api_key_registered(@api_key).deliver

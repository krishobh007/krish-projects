...
/*
 * Modified by Hitotch for Issue 550: Drop event outside of calendar / between calendars
 * Ref: https://code.google.com/p/fullcalendar/issues/detail?id=550
 * This is just a sample for our discussion.
 */
		...
		$(document)
			...
			.bind('dragstop', function(ev, ui) {
				if (_dragElement) {
					...
					currentView.dragStop(_dragElement, ev, ui,options.nameID);
					_dragElement = null;
				}
			});
	}
...
function EventManager(options, _sources) {
	...
	function renderEvent(event, stick, lost) {
		normalizeEvent(event);
		if (!event.source) {
			if (stick) {
...
function BasicView(element, calendar, viewName) {
	...
	function dragStop(_dragElement, ev, ui,nameid) {
		var cell = hoverListener.stop();
		clearOverlays();
		if (cell) {
			var d = cellToDate(cell);
			//trigger('drop', _dragElement, d, true, ev, ui);
			trigger('drop', _dragElement, d, true, ev, ui,nameid);
		}
	}
...
function DayEventRenderer() {
	...
	function draggableDayEvent(event, eventElement) {
		...
		eventElement.data('eventObject', eventObject);
